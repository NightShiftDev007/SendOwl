"""Create durable content-addressed OASIS platform-smoke orchestration.
    ...
    op.execute(
        ...
        BEGIN
            ...
            IF selected_scenario.scenario_sha256 IS DISTINCT FROM NEW.scenario_sha256
               ...
               OR selected_scenario.snapshot_company_name IS DISTINCT FROM NEW.company_name
            THEN
                RAISE EXCEPTION
    ...
    op.execute(
        ...
        BEGIN
            IF TG_OP <> 'INSERT' THEN
                RAISE EXCEPTION
                    'simulation run posts are immutable; % is forbidden', TG_OP
                    USING ERRCODE = '55000';
            END IF;
            SELECT * INTO parent
