...
def _load_default_scenarios() -> Dict[str, Any]:
    """可选加载模板 scenarios；缺失时返回空结构，保持纯函数可用。"""
    candidates = [
        ROOT.parent / "templates" / "opinion_propagation_scenarios.json",
        ROOT / "scenarios.json",
    ]
    for p in candidates:
        if p.exists():
            try:
                ...
                continue
    return {"scenarios": [], "case_id": "", "case_title": ""}
...
def llm_summarize(metrics_list: List[Dict[str, Any]]) -> Dict[str, str]:
    ...
    # try LLM polish via app Config (no hard dependency on MiroFish/.env or OUTPUTS)
    try:
        from app.config import Config as _Cfg

        key = _Cfg.LLM_API_KEY
        base = _Cfg.LLM_BASE_URL
...
def collect_live_dirs() -> List[Tuple[str, str, Path]]:
    ...
    for d in OUTPUTS.glob("demo_*"):
        if _dir_has_live_data(d):
            ...
            name = next(
                (s["name"] for s in SCENARIOS.get("scenarios", []) if s["id"] == sid),
                sid,
            )
...
def collect_synthetic(outputs_dir: Optional[Path] = None) -> List[Tuple[str, str, Path]]:
...
def run(synthetic: bool = False, outputs_dir: Optional[Path] = None) -> Path:
    out_root = Path(outputs_dir) if outputs_dir else OUTPUTS
    out_root.mkdir(parents=True, exist_ok=True)
    color_map = {
        s["id"]: s.get("color", "#333") for s in SCENARIOS.get("scenarios", [])
    }
    items = collect_synthetic(out_root) if synthetic else collect_live_dirs()
    if not items and not synthetic:
        print("[metrics] 未找到真模拟产物，回退到 synthetic")
        items = collect_synthetic(out_root)
        synthetic = True
    if not items:
        raise SystemExit("没有可计算的数据。请先 synthesize 或完成真模拟。")
    ...
    for sid, name, d in items:
        ...
        (out_root / f"metrics_{sid}.json").write_text(
    ...
    demo_data = {
        "case": {
            "id": SCENARIOS.get("case_id", ""),
            "title": SCENARIOS.get("case_title", ""),
            "mode": "synthetic" if synthetic else "live",
        },
        "scenarios": metrics_list,
        "acceptance": acceptance,
        "generated_from": "app.decision.metrics_core",
    }
    out = out_root / "demo_data.json"
    out.write_text(json.dumps(demo_data, ensure_ascii=False, indent=2), encoding="utf-8")
    (out_root / "acceptance.json").write_text(
