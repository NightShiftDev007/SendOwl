...
from app.ontology.zep_entity_reader import ZepEntityReader
...
from app.engine.simulation_runner import SimulationRunner, RunnerStatus
...
logger = get_logger('adc.api.simulation')
...
def _load_sim_config(simulation_id: str) -> dict:
    """读取 simulation_config.json（若存在）"""
    import json
...
def _resolve_simulation_requirement(data: dict, state) -> str:
    """从请求体 / ontology / config 解析模拟需求（本体完整需求优先于截断预写）"""
    req = (data.get('simulation_requirement') or '').strip()
    if req:
        return req
    ...
    try:
        ...
        ont = registry.get_ontology(state.project_id)
        if not ont:
            from app.ontology.registry import get_decision
            dec = get_decision(state.project_id)
            if dec:
                ont = registry.get_ontology(dec.get('ontology_id') or '')
        if ont:
            ont_req = (ont.get('simulation_requirement') or '').strip()
    except Exception as e:
        logger.debug(f"resolve requirement ontology: {e}")

    cfg = _load_sim_config(state.simulation_id)
    cfg_req = (cfg.get('simulation_requirement') or '').strip()

    # 预写 config 常被截成 name 前 40 字；有本体完整需求时以本体为准
    if ont_req and (not cfg_req or len(cfg_req) < len(ont_req) * 0.6):
        return ont_req
    if cfg_req:
        return cfg_req
    return ont_req
...
def _resolve_document_text(data: dict, state) -> str:
    """从请求体 / ontology 文档解析背景文本"""
    text = (data.get('document_text') or '').strip()
    if text:
        return text
    try:
        ...
        if ontology_id:
            return _combined_document_text(ontology_id) or ''
    except Exception as e:
        logger.debug(f"resolve document_text fallback: {e}")
    return ''
...
def create_simulation():
    ...
    try:
        ...
        # 终局：project_id 可为 ontology_id / decision_id（外部关联），graph_id 必填
        project_id = data.get('project_id') or data.get('ontology_id') or data.get('decision_id') or 'standalone'
        ...
        if not graph_id:
            ...
            try:
                from app.ontology import registry
                registry.init_schema()
                ont = registry.get_ontology(project_id)
                if ont:
        ...
        # 可选：把 simulation_requirement 写入 config 占位，供后续 prepare/report 使用
        req = (data.get('simulation_requirement') or '').strip()
        if req:
            import json
            cfg_path = os.path.join(state.simulation_dir if hasattr(state, 'simulation_dir') else os.path.join(Config.OASIS_SIMULATION_DATA_DIR, state.simulation_id), 'simulation_config.json')
            # state may not have simulation_dir; use Config path
            cfg_path = os.path.join(Config.OASIS_SIMULATION_DATA_DIR, state.simulation_id, 'simulation_config.json')
            existing = {}
            if os.path.exists(cfg_path):
                try:
                    ...
                    existing = {}
            existing['simulation_requirement'] = req
...
def prepare_simulation():
    ...
    try:
        ...
        # 终局：从请求体 / config / ontology 解析，不再依赖 ProjectManager
        simulation_requirement = _resolve_simulation_requirement(data, state)
        ...
        document_text = _resolve_document_text(data, state)
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    # reports 目录路径
    reports_dir = getattr(Config, 'REPORTS_DIR', None) or os.path.join(Config.UPLOAD_FOLDER, 'reports')
    ...
    try:
        ...
        for sim in simulations:
            ...
            try:
                from app.ontology import registry
                registry.init_schema()
                ont = registry.get_ontology(sim.project_id)
                if not ont:
                    from app.ontology.registry import get_decision
                    dec = get_decision(sim.project_id)
                    if dec:
                        ont = registry.get_ontology(dec.get('ontology_id') or '')
                if ont:
                    docs = registry.list_documents(ont.get('id') or sim.project_id)
                    sim_dict["files"] = [
                        {"filename": d.get('original_filename') or d.get('filename') or d.get('name') or '未知文件'}
                        for d in (docs or [])[:3]
                    ]
            except Exception:
                sim_dict["files"] = []
...
def start_simulation():
    ...
    请求（JSON）：
        {
            ...
            "no_wait": false                       // 可选: 轮次结束后立即退出，不进入采访等待模式
        }
    ...
    try:
        ...
        no_wait = bool(data.get('no_wait', False))  # 可选：轮次结束后立即退出
        ...
        if enable_graph_memory_update:
            ...
            graph_id = state.graph_id or data.get('graph_id')
            if not graph_id:
                try:
                    from app.ontology import registry
                    registry.init_schema()
...
def get_simulation_posts(simulation_id: str):
    ...
    try:
        ...
        if not os.path.exists(db_path):
            # 兼容 twitter/reddit 子目录布局
            alt = os.path.join(sim_dir, platform, db_file)
...
def get_simulation_comments(simulation_id: str):
    ...
    try:
        ...
        if not os.path.exists(db_path):
            alt = os.path.join(sim_dir, "reddit", "reddit_simulation.db")
            if os.path.exists(alt):
                db_path = alt
