...
from app.decision.report_agent import ReportAgent, ReportManager, ReportStatus
...
logger = get_logger('adc.api.report')

def _load_sim_requirement(simulation_id: str, state=None, data=None):
    """返回 (graph_id, simulation_requirement)，不依赖 ProjectManager"""
    data = data or {}
    graph_id = (state.graph_id if state else None) or data.get('graph_id') or ''
    requirement = (data.get('simulation_requirement') or '').strip()

    # config
    cfg_path = os.path.join(Config.OASIS_SIMULATION_DATA_DIR, simulation_id, 'simulation_config.json')
    ...
    # ontology / decision via project_id
    if state and (not graph_id or not requirement):
        try:
            ...
            if ont:
                ...
                if not requirement:
                    requirement = (ont.get('simulation_requirement') or '').strip()
        except Exception as e:
            logger.debug(f"report resolve fallback: {e}")

    return graph_id, requirement
...
def generate_report():
    ...
    try:
        ...
        graph_id, simulation_requirement = _load_sim_requirement(simulation_id, state, data)
        if not graph_id:
            return jsonify({
                "success": False,
...
def search_graph_tool():
    ...
    try:
        ...
        from app.ontology.zep_tools import ZepToolsService
