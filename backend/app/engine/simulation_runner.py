...
from app.ontology.zep_graph_memory_updater import ZepGraphMemoryManager
from app.engine.simulation_ipc import SimulationIPCClient, CommandType, IPCResponse
...
class SimulationRunner:
    ...
    def get_run_state(cls, simulation_id: str) -> Optional[SimulationRunState]:
        ...
        模拟子进程会直接写 run_state.json 进度心跳；因此每次读取时
        都与磁盘合并，避免内存缓存把 current_round 一直盖成 0。
        """
        disk = cls._load_run_state(simulation_id)
        mem = cls._run_states.get(simulation_id)

        if mem and disk:
            cls._merge_progress_from_disk(mem, disk)
            # 进程存活信息以内存为准，进度以较大值为准（已在 merge 中处理）
            return mem
        if mem:
            return mem
        if disk:
            cls._run_states[simulation_id] = disk
            return disk
        return None

    @classmethod
    def _merge_progress_from_disk(cls, mem: "SimulationRunState", disk: "SimulationRunState"):
        """把磁盘上的进度心跳合并进内存状态（取较大值）。"""
        if disk.current_round > mem.current_round:
            mem.current_round = disk.current_round
        if disk.twitter_current_round > mem.twitter_current_round:
            mem.twitter_current_round = disk.twitter_current_round
        if disk.reddit_current_round > mem.reddit_current_round:
            mem.reddit_current_round = disk.reddit_current_round
        if disk.twitter_actions_count > mem.twitter_actions_count:
            mem.twitter_actions_count = disk.twitter_actions_count
        if disk.reddit_actions_count > mem.reddit_actions_count:
            mem.reddit_actions_count = disk.reddit_actions_count
        if disk.updated_at and (not mem.updated_at or disk.updated_at > mem.updated_at):
            mem.updated_at = disk.updated_at
        # 子进程可能已把状态写成 completed
        if (
            mem.runner_status == RunnerStatus.RUNNING
            and disk.runner_status == RunnerStatus.COMPLETED
        ):
            mem.runner_status = RunnerStatus.COMPLETED
            mem.completed_at = disk.completed_at or mem.completed_at
            mem.twitter_running = False
            mem.reddit_running = False
    ...
    def start_simulation(
        ...
        no_wait: bool = False,  # 模拟轮次结束后立即退出，不进入采访等待模式
    ) -> SimulationRunState:
        """
        启动模拟
        ...
        Args:
            ...
            no_wait: True 时传 --no-wait，轮次结束后进程退出（适合批量编排）
        ...
        try:
            ...
            # 批量编排场景：轮次结束后立即退出，避免卡在 IPC 等待采访模式
            if no_wait:
                cmd.append("--no-wait")
            
            # 创建主日志文件，避免 stdout/stderr 管道缓冲区满导致进程阻塞
            main_log_path = os.path.join(sim_dir, "simulation.log")
            # errors='replace'：避免 emoji/代理对导致日志写入失败
    ...
    def _monitor_simulation(cls, simulation_id: str, locale: str = 'zh'):
        ...
        try:
            while process.poll() is None:  # 进程仍在运行
                ...
                # 合并子进程写入的进度心跳，避免把 current_round 盖回 0
                disk = cls._load_run_state(simulation_id)
                if disk:
                    cls._merge_progress_from_disk(state, disk)
