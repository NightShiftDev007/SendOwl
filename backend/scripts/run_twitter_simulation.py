...
class UnicodeFormatter(logging.Formatter):
    """自定义格式化器，将 Unicode 转义序列转换为可读字符，并剔除非法代理对。"""
    ...
    def format(self, record):
        try:
            ...
            # 极端情况下 record 本身含非法字符，降级为安全字符串
            result = f"{record.levelname} - {record.name} - {getattr(record, 'msg', '')}"

        def replace_unicode(match):
            try:
                ch = chr(int(match.group(1), 16))
                # 跳过 surrogate code points，避免后续 encode 失败
...
def setup_oasis_logging(log_dir: str):
    ...
    for logger_name, log_file in loggers_config.items():
        ...
        file_handler = logging.FileHandler(
            log_file, encoding='utf-8', mode='w', errors='replace'
        )
        file_handler.setLevel(logging.DEBUG)
        file_handler.setFormatter(formatter)
        logger.addHandler(file_handler)
        # 避免再向 root/console 传播：console 编码失败会刷屏 Logging error
...
try:
    ...
    PlatformActionLogger = None  # type: ignore
...
def _write_progress_heartbeat(
    ...
    *,
    ...
    """写入轻量进度，供 API / run_scenarios 轮询（不依赖 actions.jsonl）。"""
    path = os.path.join(simulation_dir, "run_state.json")
    data: Dict[str, Any] = {}
    ...
    data.update(
        {
            ...
            "twitter_actions_count": actions_count,
            "total_actions_count": actions_count,
            "progress_percent": round(current_round / max(total_rounds, 1) * 100, 1),
            "updated_at": datetime.now().isoformat(),
            "twitter_running": status == "running",
            "twitter_completed": status in ("completed", "stopped"),
        }
    )
    try:
    ...
    async def run(self, max_rounds: int = None):
        ...
        if max_rounds is not None and max_rounds > 0:
            ...
            if total_rounds < original_rounds:
                ...
                print(f"\n轮数已延长: {original_rounds} -> {total_rounds} (max_rounds={max_rounds})")
        ...
        self.env = oasis.make(
            ...
            semaphore=8,  # Demo/限流友好：降低并发，减少 429 与挂起概率
        )
        ...
        # 动作日志（供后端监控 / Demo 指标）
        action_logger = None
        if PlatformActionLogger is not None:
            action_logger = PlatformActionLogger("twitter", self.simulation_dir)
            action_logger.log_simulation_start(self.config)

        agent_names = {
            int(a.get("agent_id", i)): a.get("entity_name") or a.get("name") or f"Agent_{a.get('agent_id', i)}"
            for i, a in enumerate(self.config.get("agent_configs", []))
        ...
        for round_num in range(total_rounds):
            ...
            # 执行动作（单轮超时，避免某一轮 LLM 挂死拖垮整次模拟）
            try:
                await asyncio.wait_for(self.env.step(actions), timeout=300)
            except asyncio.TimeoutError:
                print(f"  警告: Round {round_num + 1} 超时（300s），跳过本轮")
            except Exception as e:
        ...
        _write_progress_heartbeat(
            ...
            total_rounds=total_rounds,
            actions_count=total_actions,
            status="completed" if not self.wait_for_commands else "running",
        )
