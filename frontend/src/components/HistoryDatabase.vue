...
const formatSimulationId = (simulationId) => {
  if (!simulationId) return 'TASK_UNKNOWN'
  if (simulationId.startsWith('dec_')) return `DEC_${simulationId.slice(4, 10).toUpperCase()}`
  if (simulationId.startsWith('ont_')) return `ONT_${simulationId.slice(4, 10).toUpperCase()}`
  const prefix = simulationId.replace(/^(sim_|dec_|ont_)/, '').slice(0, 6)
  return `TASK_${prefix.toUpperCase()}`
}
