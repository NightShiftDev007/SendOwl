  ...
  <div class="env-setup-panel">
    <div class="scroll-container">
      ...
      <div class="apply-bar">
        ...
        <p class="apply-hint">
          {{ hasConfirmedOrPrepared
            ? '将在同一任务上重新应用方案并重做环境准备；此前推演与报告会失效'
              ...
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                <div class="slider-display">
                  ...
                  <div class="slider-meta-info">
                    ...
                    <span v-if="customMaxRounds > autoGeneratedRounds" class="exceed-hint">已超过自动上限</span>
                  </div>
                </div>

                <label class="exceed-toggle">
                  <input type="checkbox" v-model="allowExceedAutoRounds">
                  <span>允许超过自动上限（最高 {{ ROUNDS_ABSOLUTE_MAX }}）</span>
                </label>
                ...
                <div class="range-wrapper">
                  ...
                  <div class="range-marks">
                    ...
                    <span 
                      ...
                      :style="{ position: 'absolute', left: `calc(${recommendMarkPercent}% - 30px)` }"
                    >{{ $t('step2.recommendedRounds', { rounds: 40 }) }}</span>
                    <span>{{ roundsSliderMax }}</span>
                  </div>
                </div>
              </div>
...
const applyButtonLabel = computed(() => {
  ...
  if (applyingScenarios.value) {
    return hasConfirmedOrPrepared.value ? '正在重新确认方案…' : '正在确认方案并准备…'
  }
  return hasConfirmedOrPrepared.value
    ? `重新确认方案（${n} 方案）`
    : `确认方案并准备环境（${n} 方案）`
})
...
// 模拟轮数配置
const ROUNDS_RECOMMEND = 40
const ROUNDS_ABSOLUTE_MAX = 168 // 允许超过自动上限时的硬顶（约 7 天 × 24h）
const useCustomRounds = ref(false)
const allowExceedAutoRounds = ref(false)
const customMaxRounds = ref(ROUNDS_RECOMMEND)
...
// 从配置中计算自动生成的轮数；地板 40 保留（与历史行为一致）
...
const roundsSliderMax = computed(() => {
  const auto = autoGeneratedRounds.value || ROUNDS_RECOMMEND
  if (allowExceedAutoRounds.value) {
    return Math.max(auto, ROUNDS_ABSOLUTE_MAX)
  }
  return auto
})

const roundsSliderPercent = computed(() => {
  const max = roundsSliderMax.value
  if (!max || max <= 10) return 100
  return ((customMaxRounds.value - 10) / (max - 10)) * 100
})

const recommendMarkPercent = computed(() => {
  const max = roundsSliderMax.value
  if (!max || max <= 10) return 0
  return ((40 - 10) / (max - 10)) * 100
})

watch(autoGeneratedRounds, (auto) => {
  if (!auto) return
  // 未开自定义时，内外默认保持一致（跟自动规划轮数）
  if (!useCustomRounds.value) {
    customMaxRounds.value = auto
  } else if (!allowExceedAutoRounds.value && customMaxRounds.value > auto) {
    customMaxRounds.value = auto
  }
})

watch(allowExceedAutoRounds, (allow) => {
  const auto = autoGeneratedRounds.value
  if (!allow && auto && customMaxRounds.value > auto) {
    customMaxRounds.value = auto
  }
})

watch(useCustomRounds, (on) => {
  if (!on) {
    allowExceedAutoRounds.value = false
    // 关掉自定义：恢复为自动值，避免下次打开仍停在上次拖动的数
    if (autoGeneratedRounds.value) {
      customMaxRounds.value = autoGeneratedRounds.value
    }
  }
})
...
const applyProfilesDigest = (digest, { generating = true, allowShrink = false } = {}) => {
  if (!Array.isArray(digest) || digest.length === 0) return
  // 禁止用更短 digest 盖掉已有列表（残缺 sim / 注入中途常见）
  if (!allowShrink && digest.length < (profiles.value || []).length) return
  ...
  // 生成中不标完成；完成后还须达到预期（有 expected 时）
  if (
    !generating &&
    profiles.value.length > 0 &&
    (!expectedTotal.value || profiles.value.length >= expectedTotal.value)
  ) {
    stepFlags.value.profiles = 'ok'
  }
}
...
const applyDecisionPrepareEnvelope = async (env, raw, { done = false } = {}) => {
  ...
  if (['prepare_failed', 'failed', 'error'].includes(status)) {
    stopDecisionPrepareWatch()
    // 展示后端写入的真实失败原因（prepare_progress.message）
    const progressFail =
      payload.prepare_progress?.status === 'failed' ? payload.prepare_progress?.message : ''
    const failReason = progressFail || envelope.error || payload.error || ''
    lastStageError.value = failReason || lastStageError.value
    addLog(t('log.prepareFailed', { error: failReason || '环境准备失败，请重试' }))
...
const startPrepareSimulation = async (opts = {}) => {
  ...
  // force 重准备 = 重开 Step2，后续步骤失效
  if (force) capWorkflowMaxReached(2)
...
/** 确认方案：原地替换同一 Decision 的 scenarios，再 prepare（不换卡、不进回收站） */
const applyScenariosAndPrepare = async () => {
  ...
  try {
    ...
    // 重做 Step2：立刻锁住 Step3+，避免顶栏仍可点进旧推演/报告
    capWorkflowMaxReached(2)
    ...
    const res = await replaceDecisionScenarios(decisionId, {
      title: props.projectData?.name || mapped[0]?.name,
      scenarios: mapped,
    ...
    // 与后端 created/preparing 对齐，确保顶栏不可进 Step3+
    try {
    ...
    // 放在 finally：避免首次确认末尾闪成「正在重新确认方案…」
    if (applied) hasConfirmedOrPrepared.value = true
  }
}
...
const startPolling = () => {
  ...
  const applyPrepare = async (data) => {
    ...
    if (data.status === 'completed' || data.status === 'ready' || data.already_prepared) {
      ...
      addLog(t('log.prepareFailed', { error: errMsg }))
      stopPolling()
      emit('update-status', 'error')
    }
  }
  ...
  prepareSse = subscribeTask(tid, {
    ...
    onEvent: (data) => applyPrepare(data),
    onDone: (data) => applyPrepare(data),
    onError: (err) => {
      if (settled) return
      addLog(`SSE prepare 异常，降级轮询: ${err?.message || err?.error || err}`)
      if (!pollTimer) {
...
const applyProfilesSnapshot = (data) => {
  if (!data) return
  const prevCount = profiles.value.length
  const next = Array.isArray(data.profiles) ? data.profiles : []
  // 禁止用更短列表盖掉已有人设（重开前应先清空 profiles）
  ...
  if (currentCount > 0 && !isGeneratingProfiles.value) {
    const reached =
      !expectedTotal.value || currentCount >= expectedTotal.value
    if (reached && retryingStage.value !== 'profiles') {
      stepFlags.value.profiles = 'ok'
      prepareProgress.value = 100
    } else if (!reached) {
      // 残缺完成态：进度按比例，不标 ok
      prepareProgress.value = Math.min(
        99,
        Math.round((currentCount / (expectedTotal.value || currentCount)) * 100),
      )
    }
  ...
  if (currentCount > 0 && currentCount !== lastLoggedProfileCount) {
    ...
    addLog(
      t('log.agentProfile', {
        ...
        total: total,
        name: profileName,
        profession: latestProfile?.profession || latestProfile?.entity_type || t('step2.unknownProfession'),
      }),
    )
...
const applyConfigSnapshot = (data) => {
  ...
  if (configLooksReady(data) || (data.config_generated && data.config)) {
    ...
    if (data.config.event_config?.narrative_direction) {
      ...
      addLog(
        t('log.narrativeDirection', {
          direction: narrative.length > 50 ? narrative.substring(0, 50) + '...' : narrative,
        }),
      )
    }
...
onMounted(async () => {
  ...
  try {
    ...
    if (
      ['preparing', 'prepare_failed', 'prepared', 'running', 'completed', 'ready', 'failed'].includes(
        status,
      )
    ) {
      hasConfirmedOrPrepared.value = true
    }
    if (status === 'preparing') {
    ...
    if (worldData.has_slice && !worldData.profiles?.length) {
      hasConfirmedOrPrepared.value = true
      addLog(
        `检测到未完成的世界切片（${worldData.slice_node_count || 0} 实体），尚无人设；请点击「重新确认方案」继续`,
      )
    }
