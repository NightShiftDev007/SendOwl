...
const renderGraph = () => {
  ...
  const nodesData = (props.graphData.nodes || []).map((n, i) => ({
    ...n,
    uuid: n.uuid || n.id || `n${i}`,
    name: n.name || n.label || n.uuid || n.id || `n${i}`,
  }))
  const edgesData = (props.graphData.edges || []).map((e, i) => ({
    ...e,
    uuid: e.uuid || e.id || `e${i}`,
    source_node_uuid: e.source_node_uuid || e.source || e.from,
    target_node_uuid: e.target_node_uuid || e.target || e.to,
    name: e.name || e.edge_type || e.label || '',
  }))
