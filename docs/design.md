...
| 前端五步向导 / `GraphPanel.vue`（D3） | 图谱可视化复用；主界面重做为"决策任务"视角 | 大改 |
...
- 目录规划：MiroFish 的 `backend/app/services` 按五层架构归位重组，而非整体平移——图谱/检索/回写类（`ontology_generator`、`graph_builder`、`zep_*`）并入本体层模块；模拟执行类（`simulation_*`、`run_*_simulation`、`action_logger`）并入推演引擎模块；`report_agent` 并入决策层。MiroFish 的 Flask 五步向导 API 和前端不搬（决策中心是"决策任务"视角的新界面，仅 `GraphPanel.vue` 的 D3 可视化思路可参考）。
...
1. **回测数据集**：选哪个历史事件做第一个校准基准？需要事件前的语料 + 事件后的真实传播数据。
2. **Zep 数据自主性**：本体常驻后数据量与敏感性上升，何时启动向自建图库的迁移评估？
3. **代表性 Agent 的分布数据源**：地域 × 圈层的人口先验从哪来（公开统计够不够，是否需要采买舆情画像数据）？
4. **OASIS 改造深度**：初始网络注入与环境变量干预需要动 OASIS 内部还是包一层，需要 spike 验证。
5. **产品形态**：决策中心是独立新前端，还是 MiroFish 前端的"决策模式"？（建议独立，MiroFish 保持上游开源项目身份。）
