...
>
> 文档已拆分：详见 [README.md](./README.md)。本文件保留产品与架构总览；工程运行时细节见专题文档。
...
详见 **[mirofish-integration.md](./mirofish-integration.md)**（模块映射表、整库集成方式、前端五步保留原则）。
...
2. **政策仿真 / 机构响应**：实体加入利益团体/政策条款；**先 Org Dossier 再通知刺激再推演反应** —— 专题见 **[policy-org-simulation.md](./policy-org-simulation.md)**（与 GTV 并行规划，工程错峰）；
3. **商业决策 / GTV 成交**：产业地产 CRM 结构化底座上预测开单人、房源租售与成交时间 —— 专题见 **[gtv-deal-forecast.md](./gtv-deal-forecast.md)**（当前工程优先试点；内核为可回测统计模型，非 OASIS 发帖）。
...
- **[progress-sse.md](./progress-sse.md)** — 一阶段一 SSE、ProgressEnvelope、Task TTL
- **[crash-recovery.md](./crash-recovery.md)** — Phase C 崩溃透明恢复
